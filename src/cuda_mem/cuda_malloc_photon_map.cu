...
/*   Updated: 2017/06/05 10:28:29 by jwalsh           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cuda_malloc_photon_map(t_raytracing_tools *r)
{
	...
	if (r->scene->is_photon_mapping && r->update.photon_map == 2) 
	{
		...
		gpu_errchk(cudaMallocHost(&(r->h_d_scene->photon_list), sizeof(t_photon) * PHOTON_BOUNCE_MAX * r->scene->photon_count));
		printf("Size mallocated : %lu\n", sizeof(t_photon) * PHOTON_BOUNCE_MAX * r->scene->photon_count);
		//list of photons we gather per thread.

		//Does not need to be on Host, 
		// gpu_errchk(cudaMallocHost(&(r->h_d_scene->selected_photons), sizeof(t_selected_photon) * PHOTON_GATHER_MAX * res));
