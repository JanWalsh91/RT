...
/*   Updated: 2017/06/03 11:10:02 by tgros            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_gui_no_camera_left(t_gtk_tools *g)
{
	...
	gtk_widget_set_sensitive(GTK_WIDGET(gtk_builder_get_object(
			GTK_BUILDER(g->builder), "ButtonNextCamera")), false);
	gtk_widget_set_sensitive(GTK_WIDGET(gtk_builder_get_object(
			GTK_BUILDER(g->builder), "ButtonPreviousCamera")), false);
	gtk_widget_set_sensitive(GTK_WIDGET(gtk_builder_get_object(
			GTK_BUILDER(g->builder), "ButtonRender")), false);
