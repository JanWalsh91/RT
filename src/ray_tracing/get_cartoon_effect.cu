...
/*   Updated: 2017/06/07 22:06:12 by jwalsh           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_edges(t_scene *scene, t_color *pixel_map, t_vec3 c, t_pt2 pix)
{
	...
	if (high_contrast(comp, c.y))
		return (set_black(&pixel_map[idx]));
	reset_values(&f, comp);
	while (++f < c.z)
		if (pix.y - f > 0 && pix.y + f < scene->res.y)
			add_colors(comp, pixel_map, idx - f * scene->res.y, idx + f * scene->res.y);
	if (high_contrast(comp, c.y))
	{
