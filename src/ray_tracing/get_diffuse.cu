...
/*   Updated: 2017/04/02 18:44:41 by tgros            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_ray *shadow_ray, t_light *light)
{
	...
	r2 = (!v_isnan(light->pos)) ?
		...
		1 / (4 * M_PI);
	new_col = v_scale(v_mult(light->col, v_scale(scene->objects[primary_ray->hit_obj].col,
		light->intensity / (4 * M_PI * r2))), ft_clampd(v_dot(shadow_ray->dir,
		v_scale(primary_ray->nhit, primary_ray->n_dir)), 0, 1));
	new_col = v_scale(new_col, scene->objects[primary_ray->hit_obj].kd);
