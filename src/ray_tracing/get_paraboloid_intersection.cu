...
/*   Updated: 2017/05/16 14:57:09 by tgros            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	int index)
{
	...
	i.q.x = v_dot(ray->dir, ray->dir) - v_dot(ray->dir, r->scene->objects[index].dir) * v_dot(ray->dir, r->scene->objects[index].dir);
	i.q.y = 2 * (v_dot(ray->dir, i.v1) - v_dot(ray->dir, r->scene->objects[index].dir) * (v_dot(i.v1, r->scene->objects[index].dir) + 2 * r->scene->objects[index].rad));
	i.q.z = v_dot(i.v1, i.v1) - v_dot(i.v1, r->scene->objects[index].dir) * (v_dot(i.v1, r->scene->objects[index].dir) + 4 * r->scene->objects[index].rad);
