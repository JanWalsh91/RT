...
void	render_with_aa(t_raytracing_tools *r)
{
	...
	r->d_pixel_map[r->idx] = filter(vec_to_col(average), r->scene->cameras->filter);
}
...
t_vec3		get_look_at(t_scene *scene)
{
	...
	gpu_assert(cudaMemcpy(&h_look_at, d_look_at, sizeof(t_vec3), cudaMemcpyDeviceToHost));
	return (h_look_at);
}
...
void		render(t_raytracing_tools *r, t_tile tile)
{
	...
	if (r->scene->is_3d)
	{
		...
		gpu_assert(cudaMemcpy(r->h_d_scene->cameras, r->scene->cameras, sizeof(t_camera), cudaMemcpyHostToDevice));
		gpu_assert((cudaMemcpy(r->d_scene, r->h_d_scene, sizeof(t_scene), cudaMemcpyHostToDevice)));
